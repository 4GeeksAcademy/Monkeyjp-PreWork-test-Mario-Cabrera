...
const inventarioSinTrampa = [];
for (let i = 0; i < inventario.length; i++) {
  if (inventario[i] != "trampa") {
    inventarioSinTrampa.push(inventario[i]);
  }
}
console.log(inventarioSinTrampa);
inventarioSinTrampa.push("super estrella");
console.log(inventarioSinTrampa);
const inventarioEnMayusculas = inventarioSinTrampa.map((mayus) => {
  return mayus.toUpperCase();
});
console.log(inventarioEnMayusculas);
const primerItem = inventarioEnMayusculas.slice(0,1);
const segundoItem = inventarioEnMayusculas.slice(1,2);
